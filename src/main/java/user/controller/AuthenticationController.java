...
public class AuthenticationController  {
    ...
    public ResponseEntity<ResultDTO> register(@RequestBody RegisterUserDto registerUserDto) {
        ...
        return ResponseEntity.status(HttpStatus.OK).body(new ResultDTO(200, " ثبت نام شما موفق بود", null, false));
    }
    @PostMapping("/login")
    public ResponseEntity<ResultDTO> login(@RequestBody LoginUserDto loginUserDto) {
        ...
        return ResponseEntity.status(HttpStatus.OK).body(new ResultDTO(200, "احراز هویت شما موفق بود", data, false));
    }
    @JwtAuth
    @SecurityRequirement(name = "bearerAuth")
    @PostMapping("/authoriza")
    public ResponseEntity<ResultDTO> authoriza() {
        return ResponseEntity.status(HttpStatus.OK).body(new ResultDTO(200, "احراز هویت شما موفق بود", null, false));
    }
}
